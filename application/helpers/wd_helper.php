...
if ( ! function_exists('element')) {
    ...
    function set_permalink($content, $id)
    {
        $karakter = array ('{','}',')','(','|','`','~','!','@','%','$','^','&','*','=','?','+','-','/','\\',',','.','#',':',';','\'','"','[',']');
	...
	function status_post($code){
		if ($code==0) {
			$res = '<span class="badge badge-primary">Correction</span>';
		}elseif ($code==1) {
			$res = '<span class="badge badge-warning">Review</span>';
		}elseif ($code==2) {
			$res = '<span class="badge badge-success">Published</span>';
		}
		return $res;
	}

	function telegram($msg,$html=false,$chatID='-309361641') {
		$html = $html ? "&parse_mode=HTML" : '';
		$token = 'bot275423542:AAFiS19OJRzOQ6Iq7l9Y7UvZ2lgbqJtAl2g';
	...
	function string_playtime($detik=0){
		$ci = &get_instance();		
		$ci->load->library('get_id3');
		return getid3_lib::PlaytimeString($detik);
	}

	function modul_aktif($val='',$uri=1){
		$ci = &get_instance();		
		return ($ci->uri->segment($uri)==$val) ? 'active' : '';
	}
	
	function paging($link,$jml,$uri=5){

			$config['base_url'] 	= $link;
			$config['total_rows'] 	= $jml;
			$config['uri_segment'] 	= $uri;
			$config['per_page'] 	= 10;
			$config['cur_tag_open']='<li class="active" ><span class="page-numbers active">';
    ...
    function random_number($lenght=4,$all=FALSE) {
        $alphabet = '1234567890';
        if($all) $alphabet = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
        $pass = array(); 
	...
	function encrypt_url($string) {
	  ...
	  // $key = "ParTTwo"; //key to encrypt and decrypts.
	  $result = '';
	  $test = "";
	...
	function check_files($name,$redirect,$size,$allowed,$return=false){
		$ci = &get_instance();
		$ext = pathinfo($_FILES[$name]['name'],PATHINFO_EXTENSION);
		if(! in_array($ext,$allowed)){				
			$ci->session->set_flashdata('danger_message', 'File type not allowed !!');
			
			if($return) return 'File type not allowed !!';
			else redirect(admin_dir().this_module().$redirect);
			exit();
		}
		...
		if ($filesize < $min_size ){
			...
			if($return) return 'File terlalu kecil !!';
			else redirect(admin_dir().this_module().$redirect);
			exit();
		}
		if ($filesize > $max_size){
			...
			if($return) return 'File terlalu besar !!';
			else redirect(admin_dir().this_module().$redirect);
			exit();
		}
		return false;
