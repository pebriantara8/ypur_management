...
/* https://browserleaks.com/js/webgl.js */
$(function() {
    function a(b, m) {
        if (q) var n = performance.now();
        void 0 === b && (b = !1);
        var u = 0,
            v = 0;
        ...
        if (1 != u || x || (u = 2), 1 == v) {
            var y = !1;
            for (var z in m) "2" == m[z].slice(-1) && (y = !0);
            y || (v = 2)
        }
        var A = !1;
        if (q) {
        ...
        if (x) {
            var C = x.gl;
            if ("2" == x.name[0].slice(-1)) A = 2, $("#webgl-table tbody.w2").removeClass("n"), $("#webgl-table tbody.w1").addClass("n");
            else {
            ...
            for (var z in J) {
                ...
                if (J[z] instanceof Array) {
                    ...
                    L = "[" + L + "]"
                } else L = C.getParameter(C[J[z]]), null === L ? L = "n/a" : "object" == typeof L && null != L && (L = d(L));
                $("#f" + z).text(L)
            }
            var N = "";
            for (var z in m) "" != N && (N += ", "), m[z] != x.name[0] ? N += '<span class="href" id="switch-' + m[z] + '" title="switch to &quot;' + m[z] + '&quot;">' : m.length > 1 && (N += "<strong>"), N += m[z], m[z] != x.name[0] ? N += "</span>" : m.length > 1 && (N += "</strong>");
            if ($("#f_name").html(N), q) {
                var O = performance.now();
                console.log("t4-t3", O - I)
            }
            for (var z in m) $("#switch-" + m[z]).click(function(b) {
                b.preventDefault(), $(this).off("click"), a($(this).attr("id").slice(7), m)
            });
            $("#f_alias").text(e(C));
            var P = f(C);
            $("#u_vendor").html(P.vendor), $("#u_renderer").html(P.renderer), $("#f_angle").text(g(C)), $("#f_anisotropy").text(h(C)), $("#f_caveat").text(i(x.name[0])), 1 == A && $("#f_max_draw_buffers").text(j(C)), $("#f_float_int").text(k(C)), $("#f_ext").html(l(C)), $("#f_vertext").html(o(C, C.VERTEX_SHADER)), $("#f_fragment").html(o(C, C.FRAGMENT_SHADER)), $(".ext-link").each(function() {
                $(this).on("mouseover", function() {
                    $(this).off();
                    var a = ext_link = $(this).first().text();
                    "WEBKIT_lose_context" === ext_link ? ext_link = "WEBGL_lose_context" : "WEBKIT_WEBGL_compressed_textures" === ext_link && (ext_link = ""), ext_link = ext_link.replace(/^WEBKIT_|MOZ_|_EXT_/, ""), $(this).html('<a href="https://www.khronos.org/registry/webgl/extensions/' + ext_link + '" title="' + ("" != ext_link ? ext_link += " — " : "") + 'WebGL Extension Specification" rel="noopener nofollow" target="_blank">' + a + "</a>")
                })
            }), window.location.hash && !b && clck(), 1 == A ? $(".w1").addClass("w1only") : 2 == A && $(".w2").addClass("w2only"), p(C)
        } else $("#webgl-table").removeClass("script").addClass("opac"), $("#webgl-table em.ns").removeClass("ns");
        if (q) {
            var Q = performance.now();
            console.log("t5-t4", Q - O)
        }
        if ($("#webgl1-status").html(t[u]), $("#webgl2-status").html(t[v] + (1 == v ? webgl2_support_functions : "")), A && "" != webgl2_support_functions) {
            var R = $("#webgl2-status input"),
                S = $("#webgl2-tbody");
            R.click(function(a) {
                "more" == R.attr("value") ? (R.attr("value", "less"), S.removeClass("n")) : (R.attr("value", "more"), S.addClass("n"))
    ...
    function b(a) {
        var b = "",
            c = $("#howto-enable-disable-webgl").next();
    ...
    function l(a) {
        ...
        if (void 0 !== b && b.length) {
            ...
            if (c += "<tr><td>Supported Privileged Extensions</td>", d.length > 0)
                for (var e in d) e > 0 && (c += '<tr><td class="nt"></td>'), c += '<td class="ext-link"><span class="href">' + d[e] + "</span></td></tr>";
            else c += "<td>n/a</td></tr>"
    ...
    var q = !1,
        ...
        t = [s + "False", r + "True", s + "False (supported, but disabled in browser settings, or blocked by extensions)", s + "False (supported, but blocked by browser extensions)"];
    webgl2_support_functions = "", a(),
        function() {
            setTimeout(function() {
                var a;
                try {
                    var b = document.createElement("canvas");
                    b.width = 256, b.height = 128, a = b.getContext("webgl2") || b.getContext("experimental-webgl2") || b.getContext("webgl") || b.getContext("experimental-webgl") || b.getContext("moz-webgl")
                } catch (a) {
                    q && console.warn("WebGL Image Hash", a)
                }
                if (null == a) return !1;
                // var webgl_report_hash = c;
                ...
                try {
                    ...
                    if (a.readPixels(0, 0, 256, 128, a.RGBA, a.UNSIGNED_BYTE, j), i = JSON.stringify(j).replace(/,?"[0-9]+":/g, ""), "" == i.replace(/^{[0]+}$/g, "")) throw err = "JSON.stringify only ZEROes";
        ...
        }(), void 0 !== window.chrome ? b("chrome") : navigator.userAgent.indexOf("Safari/") > -1 ? b("safari") : (document.documentMode || navigator.userAgent.indexOf("Edge") > -1) && b("msie")
});